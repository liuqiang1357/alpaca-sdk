...
import IPancakePair from '@alpaca-finance/pancake-swap-core/build/IPancakePair.json';
...
var Trade = /*#__PURE__*/function () {
  ...
  Trade.exactIn = function exactIn(route, amountIn, tradingFeeBasisPoint) {
    return new Trade(route, amountIn, TradeType.EXACT_INPUT, tradingFeeBasisPoint);
  }
  /**
  ...
  Trade.exactOut = function exactOut(route, amountOut, tradingFeeBasisPoint) {
    return new Trade(route, amountOut, TradeType.EXACT_OUTPUT, tradingFeeBasisPoint);
  }
  /**
  ...
  currentPairs, originalAmountIn, bestTrades) {
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      if (amountOut.token.equals(tokenOut)) {
        sortedInsert(bestTrades, new Trade(new Route([].concat(currentPairs, [pair]), originalAmountIn.currency, currencyOut), originalAmountIn, TradeType.EXACT_INPUT, tradingFeeBasisPoint), maxNumResults, tradeComparator);
  ...
  currentPairs, originalAmountOut, bestTrades) {
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      if (amountIn.token.equals(tokenIn)) {
        sortedInsert(bestTrades, new Trade(new Route([pair].concat(currentPairs), currencyIn, originalAmountOut.currency), originalAmountOut, TradeType.EXACT_OUTPUT, tradingFeeBasisPoint), maxNumResults, tradeComparator);
      } else if (maxHops > 1 && pairs.length > 1) {
...
//# sourceMappingURL=pancakeswap-sdk.esm.js.map
